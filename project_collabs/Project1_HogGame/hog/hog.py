...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i <= num_rolls:
        ...
        if roll > 1:
            ...
            rolls_sum = 1           #the pig out rule triggered
            break
    return rolls_sum
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    score = 0
    if num_rolls == 0:              #Free Bacon play
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < goal and opponent_score < goal:
        #print('Turn: ', turn)
        #print('Score0: ', score)
        #print('Score1: ', opponent_score)
        if who == 0:
            #print('Player0 rolls...')
            score += take_turn(strategy0(score, opponent_score), opponent_score, select_dice)
            ...
            #print('Player1 rolls...')
            opponent_score += take_turn(strategy1(opponent_score, score), score, select_dice)
            if score*2 == opponent_score or opponent_score*2 == score:
                swap = score
                score = opponent_score
                opponent_score = swap
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    while i < 11:
        ...
        print(i, 'dice scores', avg, 'on average')
        if avg > max_average_score:
            max_average_score = avg
            choice = i
        i += 1
    return choice
...
def swap_strategy(score, opponent_score):
    ...
    5
    """
    possible_freebacon = 1 + max(opponent_score//10, opponent_score%10)
    
    if 2*(score + possible_freebacon) == opponent_score:
        return 0
    elif (score + possible_freebacon) == 2*opponent_score:
...
def final_strategy(score, opponent_score):
    ...
    This strategy is similar to swap, except the baseline number of rolls is 
    changed depending on whether or not a lead is held. If in the lead,
    higher baseline number of rolls and bacon margin. Conversly, if lagging,
    the lower the number of baseline rolls and bacon margin. 

    Performance is volitile and I did not ever get .59 win rate over the baseline
    strategy, but win rates of 0.57 crop up with a higher frequency for sure. 
    """
    possible_freebacon = 1 + max(opponent_score//10, opponent_score%10)
    
    if 2*(score + possible_freebacon) == opponent_score:
        return 0
    elif (score + possible_freebacon) == 2*opponent_score: # in lead, less risk
        return 5 
   
    if score < opponent_score:
        if possible_freebacon >= 9: #behind, more risk
            return 0
        else:
            return 7
    else:
        if possible_freebacon >= 7: #ahead, easy points, less risk
            return 0
        else:
            return 5
