...
def about(topic):
    ...
    def compare(p):
    	digest = split(lower(remove_punctuation(p)))
    	counter = 0
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    stop_index = len(typed_words) #interested in comparing only typed words
    if len(typed_words) > len(reference_words):
    	wrong = len(typed_words) - len(reference_words)
    	stop_index = len(reference_words) #unless the words typed exceed the reference
    ...
    for i in range(0, stop_index):
    	if reference_words[i] != typed_words[i]:
            wrong += 1
    wrong_proportion = wrong/len(typed_words)
    right_proportion = 1 - wrong_proportion
    percent_match = right_proportion * 100
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    word_groups = len(typed)/5
    return word_groups* (60/elapsed)
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        #make list of pairs with the difference at each index of valid_words
        differences = []
        for i in range(0,len(valid_words)):
            differences.append((i,diff_function(typed_word, valid_words[i], limit)))

        #find items in valid_words with min difference
        mins = [min(differences, key=lambda pairs: pairs[1])]
...
def sphinx_switches(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    difference = abs(len(start)-len(goal))
    if difference > 0:
        shorter = min(start,goal, key = len)
        if shorter == start:
            start2 = start
            goal2 = goal[:-difference]
        elif shorter == goal:
            start2 = start[:-difference]
            goal2 = goal
        else:
    ...
    def helper(w1, w2, diff):
        if w1 == ''  or w2 == '':
            return diff
        #stop recursion once limit has been surpassed (by 1)
        elif diff > limit:
            return diff
        elif w1[0] != w2[0]:
    ...
    return helper(start2, goal2, difference)
    # END PROBLEM 6
