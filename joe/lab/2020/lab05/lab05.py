...
def distance(city_a, city_b):
    ...
    5.0
    """
    lat_dist = (get_lat(city_a) - get_lat(city_b))**2
    lon_dist = (get_lon(city_a) - get_lon(city_b))**2
...
def closer_city(lat, lon, city_a, city_b):
    ...
    if(distance(city_a, test_city) > distance(city_b, test_city)):
...
def riffle(deck):
    ...
    """
    return [deck[int(k/2)] if(k % 2 == 0) else deck[int((len(deck)/2 + ((k-1)/2)))] for k in range(len(deck))]
